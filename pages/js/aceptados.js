...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://4e85bfc27ed8.ngrok-free.app/api/embargo/aceptados';
...
const mostrar = (clientes) => {
    ...
    clientes.forEach((cliente) => {
        ...
        if (cliente.ruta_desprendible && cliente.ruta_desprendible !== '') {
            botonPDF = `
            <a class="btn btn-md btn-info visualizar-pdf" href="https://4e85bfc27ed8.ngrok-free.app/uploads/${cliente.ruta_desprendible}" target="_blank">
...
document.getElementById('btnConfirmarDesprendible').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('https://4e85bfc27ed8.ngrok-free.app/api/subir-desprendible-embargos', {
            method: 'POST',
            body: formData
        });
