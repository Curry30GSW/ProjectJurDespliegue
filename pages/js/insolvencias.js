...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://4e85bfc27ed8.ngrok-free.app/api/clientes-insolvencias';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $(document).on('click', '.crear-insolvencia', function () {
        ...
        if (cliente.foto) {
            $('#fotoperfilModal').attr('src', `https://4e85bfc27ed8.ngrok-free.app${cliente.foto}`);
        } else {
...
document.getElementById('formCrearInsolvencia').addEventListener('submit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch('https://4e85bfc27ed8.ngrok-free.app/api/actualizar-insolvencias', {
                method: 'PUT',
                body: formData
            })
...
document.addEventListener('click', async function (e) {
    if (e.target.classList.contains('editar-proceso')) {
        ...
        try {
            const response = await fetch(`https://4e85bfc27ed8.ngrok-free.app/api/insolvencia/cedula/${cedula}`);
...
function cargarDatosEnFormulario(cliente) {
    ...
    if (foto) {
        foto.src = cliente.foto_perfil ? `https://4e85bfc27ed8.ngrok-free.app${cliente.foto_perfil}` : 'assets/img/avatar.png';
    ...
    if (cliente.autoliquidador) {
        ...
        filePreview.innerHTML = `
            <a href="https://4e85bfc27ed8.ngrok-free.app${cliente.autoliquidador}" target="_blank" class="btn btn-md btn-outline-info mt-2">
    ...
    if (cliente.ruta_desprendible) {
        ...
        filePreview.innerHTML = `
        <a href="https://4e85bfc27ed8.ngrok-free.app${cliente.ruta_desprendible}" target="_blank" class="btn btn-outline-info btn-md" title="Ver desprendible">
    ...
    if (cliente.acta_aceptacion) {
        ...
        filePreview.innerHTML = `
        <a href="https://4e85bfc27ed8.ngrok-free.app${cliente.acta_aceptacion}" target="_blank" class="btn btn-outline-info btn-md" title="Ver acta de aceptación">
...
function llenarModalDetalle(cliente, fotoUrl) {
    ...
    if (cliente.bienes === "1" && cliente.bienes_inmuebles) {
        bienesInmueblesDiv.innerHTML = `
         <button class="btn btn-sm btn-outline-primary" onclick="window.open('https://4e85bfc27ed8.ngrok-free.app${cliente.bienes_inmuebles}', '_blank')">
    ...
    if (cliente.datacred === "1" && cliente.data_credPdf) {
        ...
        <button class="btn btn-sm btn-outline-primary" onclick="window.open('https://4e85bfc27ed8.ngrok-free.app${cliente.data_credPdf}', '_blank')">
