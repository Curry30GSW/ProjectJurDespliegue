...
async function verificarCedula(cedula) {
  try {
    const response = await fetch(`https://0086b16377e5.ngrok-free.app/api/clientes/${cedula}`);
...
async function subirArchivo(file, tipo, cedula) {
  ...
  try {
    const response = await fetch('https://0086b16377e5.ngrok-free.app/api/upload', {  // Cambiado a ruta relativa
      method: 'POST',
      body: formData
    });
...
async function enviarDatosCliente(formValues) {
  try {
    const response = await fetch('https://0086b16377e5.ngrok-free.app/api/insert-clientes', {
      method: 'POST',
      headers: {
