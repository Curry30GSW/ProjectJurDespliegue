...
async function verificarCedula(cedula) {
  try {
    const response = await fetch(`https://little-queens-cry.loca.lt/api/clientes/${cedula}`);
...
async function subirArchivo(file, tipo, cedula) {
  ...
  try {
    const response = await fetch('https://little-queens-cry.loca.lt/api/upload', {  // Cambiado a ruta relativa
      method: 'POST',
      body: formData
    });
...
async function enviarDatosCliente(formValues) {
  try {
    const response = await fetch('https://little-queens-cry.loca.lt/api/insert-clientes', {
      method: 'POST',
      headers: {
