...
async function verificarCedula(cedula) {
  try {
    const response = await fetch(`https://cruel-rules-cheer.loca.lt/api/clientes/${cedula}`);
...
async function subirArchivo(file, tipo, cedula) {
  ...
  try {
    const response = await fetch('https://cruel-rules-cheer.loca.lt/api/upload', {  // Cambiado a ruta relativa
      method: 'POST',
      body: formData
    });
...
async function enviarDatosCliente(formValues) {
  try {
    const response = await fetch('https://cruel-rules-cheer.loca.lt/api/insert-clientes', {
      method: 'POST',
      headers: {
