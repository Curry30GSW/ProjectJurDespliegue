...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://cruel-rules-cheer.loca.lt/api/clientes-embargos';
...
function verCliente(id_embargos) {
    fetch(`https://cruel-rules-cheer.loca.lt/api/embargos/${id_embargos}`)
                ...
                <div class="col-md-4 mb-3 mb-md-0" style="padding-right: 15px;">
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; height: 100%;">
                        ...
                        <div class="text-center mb-3">
                            <img src="${datos.embargo.foto_perfil ? `https://cruel-rules-cheer.loca.lt${datos.embargo.foto_perfil}` : 'assets/img/avatar.png'}" 
                                class="rounded-circle me-3" 
                                alt="${datos.embargo.nombres}" 
                                style="width: 120px; height: 120px; object-fit: cover; border: 3px solid #dee2e6;"
                                data-src="${datos.embargo.foto_perfil ? `https://cruel-rules-cheer.loca.lt${datos.embargo.foto_perfil}` : 'assets/img/avatar.png'}">
                        </div>
...
async function editarCliente(id_embargos) {
    ...
    try {
        const response = await fetch(`https://cruel-rules-cheer.loca.lt/api/embargos/${id_embargos}`);
        ...
        document.getElementById('detalleFotoPerfil').src = data.embargo.foto_perfil
            ? `https://cruel-rules-cheer.loca.lt${data.embargo.foto_perfil}`
...
async function seleccionarEstadoFinal(estado, id_embargos) {
    ...
    try {
        const response = await fetch(`https://cruel-rules-cheer.loca.lt/api/embargo/${id_embargos}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
