...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://little-doodles-jump.loca.lt/api/clientes-insolvencias';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $(document).on('click', '.crear-insolvencia', function () {
        ...
        if (cliente.foto) {
            $('#fotoperfilModal').attr('src', `https://little-doodles-jump.loca.lt${cliente.foto}`);
        } else {
...
document.getElementById('formCrearInsolvencia').addEventListener('submit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch('https://little-doodles-jump.loca.lt/api/actualizar-insolvencias', {
                method: 'PUT',
                body: formData
            })
...
document.addEventListener('click', async function (e) {
    if (e.target.classList.contains('editar-proceso')) {
        ...
        try {
            const response = await fetch(`https://little-doodles-jump.loca.lt/api/insolvencia/cedula/${cedula}`);
...
function cargarDatosEnFormulario(cliente) {
    ...
    if (foto) {
        foto.src = cliente.foto_perfil ? `https://little-doodles-jump.loca.lt${cliente.foto_perfil}` : 'assets/img/avatar.png';
    ...
    if (cliente.autoliquidador) {
        ...
        filePreview.innerHTML = `
            <a href="https://little-doodles-jump.loca.lt${cliente.autoliquidador}" target="_blank" class="btn btn-md btn-outline-info mt-2">
    ...
    if (cliente.ruta_desprendible) {
        ...
        filePreview.innerHTML = `
        <a href="https://little-doodles-jump.loca.lt${cliente.ruta_desprendible}" target="_blank" class="btn btn-outline-info btn-md" title="Ver desprendible">
    ...
    if (cliente.acta_aceptacion) {
        ...
        filePreview.innerHTML = `
        <a href="https://little-doodles-jump.loca.lt${cliente.acta_aceptacion}" target="_blank" class="btn btn-outline-info btn-md" title="Ver acta de aceptación">
...
function llenarModalDetalle(cliente, fotoUrl) {
    ...
    if (cliente.bienes === "1" && cliente.bienes_inmuebles) {
        bienesInmueblesDiv.innerHTML = `
         <button class="btn btn-sm btn-outline-primary" onclick="window.open('https://little-doodles-jump.loca.lt${cliente.bienes_inmuebles}', '_blank')">
    ...
    if (cliente.datacred === "1" && cliente.data_credPdf) {
        ...
        <button class="btn btn-sm btn-outline-primary" onclick="window.open('https://little-doodles-jump.loca.lt${cliente.data_credPdf}', '_blank')">
