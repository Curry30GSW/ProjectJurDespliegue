...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://green-crews-hope.loca.lt/api/clientes-insolvencias';
        ...
        <tr>
            <td>
                <div class="d-flex align-items-center px-2 py-1">
                    <div>
                        <img src="https://green-crews-hope.loca.lt${cliente.foto_perfil}" 
                            class="avatar avatar-lg me-3 foto-cliente" 
                            alt="${cliente.nombres}"
                            data-src="https://green-crews-hope.loca.lt${cliente.foto_perfil}">
                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $(document).on('click', '.crear-insolvencia', function () {
        ...
        if (cliente.foto) {
            $('#fotoperfilModal').attr('src', `https://green-crews-hope.loca.lt${cliente.foto}`);
        } else {
...
document.getElementById('formCrearInsolvencia').addEventListener('submit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch('https://green-crews-hope.loca.lt/api/actualizar-insolvencias', {
                method: 'PUT',
                body: formData
            })
...
document.addEventListener('click', async function (e) {
    if (e.target.classList.contains('editar-proceso')) {
        ...
        try {
            const response = await fetch(`https://green-crews-hope.loca.lt/api/insolvencia/cedula/${cedula}`);
...
function cargarDatosEnFormulario(cliente) {
    ...
    if (foto) {
        foto.src = cliente.foto_perfil ? `https://green-crews-hope.loca.lt${cliente.foto_perfil}` : 'assets/img/avatar.png';
    ...
    if (cliente.autoliquidador) {
        ...
        filePreview.innerHTML = `
            <a href="https://green-crews-hope.loca.lt${cliente.autoliquidador}" target="_blank" class="btn btn-md btn-outline-info mt-2">
    ...
    if (cliente.ruta_desprendible) {
        ...
        filePreview.innerHTML = `
        <a href="https://green-crews-hope.loca.lt${cliente.ruta_desprendible}" target="_blank" class="btn btn-outline-info btn-md" title="Ver desprendible">
    ...
    if (cliente.acta_aceptacion) {
        ...
        filePreview.innerHTML = `
        <a href="https://green-crews-hope.loca.lt${cliente.acta_aceptacion}" target="_blank" class="btn btn-outline-info btn-md" title="Ver acta de aceptación">
...
document.querySelector('#tablaClientes tbody').addEventListener('click', function (e) {
    ...
    if (boton) {
        ...
        fetch(`https://green-crews-hope.loca.lt/api/clientes/${cedula}`)
            .then(response => response.json())
            .then(cliente => {
...
function llenarModalDetalle(cliente, fotoUrl) {
    ...
    fotoPerfil.src = cliente.foto_perfil
        ? `https://green-crews-hope.loca.lt${cliente.foto_perfil}`
    ...
    if (cliente.bienes === "1" && cliente.bienes_inmuebles) {
        bienesInmueblesDiv.innerHTML = `
         <button class="btn btn-sm btn-outline-primary" onclick="window.open('https://green-crews-hope.loca.lt${cliente.bienes_inmuebles}', '_blank')">
    ...
    if (cliente.datacred === "1" && cliente.data_credPdf) {
        ...
        <button class="btn btn-sm btn-outline-primary" onclick="window.open('https://green-crews-hope.loca.lt${cliente.data_credPdf}', '_blank')">
...
function actualizarBotonPDF(elementId, url, textoBoton) {
    ...
    if (url) {
        const fullUrl = url.startsWith('http') ? url : `https://green-crews-hope.loca.lt${url}`;
        elemento.innerHTML = `
