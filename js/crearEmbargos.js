document.addEventListener('DOMContentLoaded', function () {
    ...
    async function buscarCliente() {
        ...
        try {
            ...
            const response = await fetch(`https://cruel-rules-cheer.loca.lt/api/cliente-embargos/${cedula}`);
            ...
            if (cliente.foto_perfil) {
                clienteFotoPerfil.src = `https://cruel-rules-cheer.loca.lt${cliente.foto_perfil}`;
    ...
    function actualizarDetalleCliente(cliente) {
        ...
        if (cliente.foto_perfil) {
            fotoPerfil.src = `https://cruel-rules-cheer.loca.lt${cliente.foto_perfil}`;
        } else {
...
async function seleccionarEstadoFinal(estado) {
    ...
    try {
        const response = await fetch('https://cruel-rules-cheer.loca.lt/api/crear-embargos', {
            method: 'POST',
        ...
        if (subsanacionSI) {
            ...
            try {
                ...
                const notifRes = await fetch('https://cruel-rules-cheer.loca.lt/api/notificaciones-embargos', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
