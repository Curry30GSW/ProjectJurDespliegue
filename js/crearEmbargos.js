document.addEventListener('DOMContentLoaded', function () {
    ...
    async function buscarCliente() {
        ...
        try {
            ...
            const response = await fetch(`https://fast-papers-film.loca.lt/api/cliente-embargos/${cedula}`);
            ...
            if (cliente.foto_perfil) {
                clienteFotoPerfil.src = `https://fast-papers-film.loca.lt${cliente.foto_perfil}`;
    ...
    function actualizarDetalleCliente(cliente) {
        ...
        if (cliente.foto_perfil) {
            fotoPerfil.src = `https://fast-papers-film.loca.lt${cliente.foto_perfil}`;
        } else {
...
async function seleccionarEstadoFinal(estado) {
    ...
    try {
        const response = await fetch('https://fast-papers-film.loca.lt/api/crear-embargos', {
            method: 'POST',
        ...
        if (subsanacionSI) {
            ...
            try {
                ...
                const notifRes = await fetch('https://fast-papers-film.loca.lt/api/notificaciones-embargos', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
