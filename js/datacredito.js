...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://cruel-rules-cheer.loca.lt/api/clientes-datacredito';
...
document.getElementById('btnConfirmarDatacredito').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('https://cruel-rules-cheer.loca.lt/api/subir-documento', {
            method: 'POST',
            body: formData
        });
...
document.getElementById('btnConfirmarMoverArea').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('https://cruel-rules-cheer.loca.lt/api/mover-area', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function cargarNotificaciones() {
    try {
        const res = await fetch('https://cruel-rules-cheer.loca.lt/api/notificaciones');
