...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://little-queens-cry.loca.lt/api/clientes-datacredito';
...
document.getElementById('btnConfirmarDatacredito').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('https://little-queens-cry.loca.lt/api/subir-documento', {
            method: 'POST',
            body: formData
        });
...
document.getElementById('btnConfirmarMoverArea').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('https://little-queens-cry.loca.lt/api/mover-area', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function cargarNotificaciones() {
    try {
        const res = await fetch('https://little-queens-cry.loca.lt/api/notificaciones');
