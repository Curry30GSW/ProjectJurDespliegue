...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://pink-games-doubt.loca.lt/api/embargo/aceptados';
...
const mostrar = (clientes) => {
    ...
    clientes.forEach((cliente) => {
        ...
        if (cliente.ruta_desprendible && cliente.ruta_desprendible !== '') {
            botonPDF = `
            <a class="btn btn-md btn-info visualizar-pdf" href="https://pink-games-doubt.loca.lt/uploads/${cliente.ruta_desprendible}" target="_blank">
...
document.getElementById('btnConfirmarDesprendible').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('https://pink-games-doubt.loca.lt/api/subir-desprendible-embargos', {
            method: 'POST',
            body: formData
        });
