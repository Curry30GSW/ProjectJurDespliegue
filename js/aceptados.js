...
async function obtenerClientes() {
    try {
        const token = sessionStorage.getItem('token');
        const url = 'https://green-crews-hope.loca.lt/api/embargo/aceptados';
...
const mostrar = (clientes) => {
    ...
    clientes.forEach((cliente) => {
        ...
        if (cliente.ruta_desprendible && cliente.ruta_desprendible !== '') {
            botonPDF = `
            <a class="btn btn-md btn-info visualizar-pdf" href="https://green-crews-hope.loca.lt/uploads/${cliente.ruta_desprendible}" target="_blank">
...
document.getElementById('btnConfirmarDesprendible').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('https://green-crews-hope.loca.lt/api/subir-desprendible-embargos', {
            method: 'POST',
            body: formData
        });
