...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function obtenerClientes() {
        try {
            const token = sessionStorage.getItem('token');
            const url = 'https://cruel-rules-cheer.loca.lt/api/clientes';
...
$(document).on('click', '.editar-cliente', function () {
    ...
    $.get(`https://cruel-rules-cheer.loca.lt/api/clientes/${cedula}`, function (cliente) {
        ...
        if (cliente.foto_perfil) {
            $('#editarFotoPerfil').attr('src', `https://cruel-rules-cheer.loca.lt${cliente.foto_perfil}`);
        }
        ...
        if (cliente.desprendible) {
            $('#verDesprendible')
                ...
                .off('click')
                .on('click', function () {
                    window.open(`https://cruel-rules-cheer.loca.lt${cliente.desprendible}`, '_blank');
                });
        } else {
        ...
        if (cliente.bienes_inmuebles && cliente.bienes_inmuebles.length > 0) {
            $('#verBienesInmuebles')
                ...
                .on('click', function () {
                    ...
                    window.open(`https://cruel-rules-cheer.loca.lt${cliente.bienes_inmuebles}`, '_blank');
                });
        } else {
        ...
        if (cliente.cedula_pdf) {
            $('#verCedulaPDF')
                ...
                .off('click')
                .on('click', function () {
                    window.open(`https://cruel-rules-cheer.loca.lt${cliente.cedula_pdf}`, '_blank');
                });
        } else {
...
$('#formEditarCliente').submit(function (e) {
    ...
    $.ajax({
        url: `https://cruel-rules-cheer.loca.lt/api/clientes/${cedula}`,
        type: 'PUT',
        data: formData,
        contentType: false,
        processData: false,
